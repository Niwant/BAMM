...
if __name__ == "__main__":
    # Process the single joint file we have
    joints_file = "extracted_joints.npy"
    output_file = "extracted_joints.bvh"
    
    if os.path.exists(joints_file):
        joints = np.load(joints_file)
        converter = Joint2BVHConvertor()
        new_anim = converter.convert(joints, output_file, foot_ik=False)
        print(f"Converted joints to BVH and saved to {output_file}")
    else:
